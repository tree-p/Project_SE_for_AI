...
def _classify(model: nn.Module, img: Image.Image) -> dict:
    ...
    tensor = TRANSFORM(img).unsqueeze(0)  # [1, 3, 299, 299]
    ...
    # รวม Half-Fresh + Spoiled = "ไม่สด"
    if predicted_class == "Fresh":
        freshness = "สด"
        advice    = "ก่อนเก็บเนื้อวัวควรล้างให้สะอาด จากนั้นซับให้แห้ง เก็บในกล่องเก็บอาหารที่มีฝาปิดสนิทแล้วแช่เนื้อวัวในตู้เย็นซึ่งอยู่ได้ 3-4 วัน แต่หากแช่ในช่องฟรีซอยู่ได้นาน 4-6 เดือน"
    else:
        freshness = "ไม่สด"
